...
public class GameScreen implements Screen{
    ...
    public GameScreen(final LateCheckin gam) {
        ...
        bucket.y = Const.Bucket.aboveBottom; // bottom left corner of the bucket is 20 pixels above
    ...
    public void render(float delta) {
        ...
        if (Gdx.input.isKeyPressed(Keys.LEFT))
            bucket.x -= Const.Bucket.slideWidth * Gdx.graphics.getDeltaTime();
        ...
        if (bucket.x < 0)
            bucket.x = 0;
        if (bucket.x > Const.Viewport.width - Const.Bucket.width)
            bucket.x = Const.Viewport.width - Const.Bucket.width;
        ...
        while (iter.hasNext()) {
            ...
            if (raindrop.y + Const.Raindrop.height < 0)
                iter.remove();
